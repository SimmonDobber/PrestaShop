 ...
 * @license   https://opensource.org/licenses/OSL-3.0 Open Software License (OSL 3.0)
 */
const{$:S}=window;class ${extend(t){this.initRowLinks(t),this.initConfirmableActions(t)}initConfirmableActions(t){t.getContainer().on("click",e.linkRowAction,(t=>{const e=S(t.currentTarget).data("confirm-message");e.length&&!window.confirm(e)&&t.preventDefault()}))}initRowLinks(t){S("tr",t.getContainer()).each((function(){const t=S(this);S(e.linkRowActionClickableFirst,t).each((function(){const n=S(this),o=n.closest("td"),i=S(e.clickableTd,t).not(o);let r=!1;i.addClass("cursor-pointer").mousedown((()=>{S(window).mousemove((()=>{r=!0,S(window).unbind("mousemove")}))})),i.mouseup((()=>{const t=r;if(r=!1,S(window).unbind("mousemove"),!t){const t=n.data("confirm-message");(!t.length||window.confirm(t)&&n.attr("href"))&&(document.location.href=n.attr("href"))}}))}))}))}}
/**
 * Copyright since 2007 PrestaShop SA and Contributors
 * PrestaShop is an International Registered Trademark & Property of PrestaShop SA
 *
 ...
 * @license   https://opensource.org/licenses/OSL-3.0 Open Software License (OSL 3.0)
 */
const{$:jt}=window;jt((()=>{const t=new k("order_message");t.addExtension(new C),t.addExtension(new B),t.addExtension(new M),t.addExtension(new L),t.addExtension(new $),t.addExtension(new xt),t.addExtension(new I),t.addExtension(new Et),t.addExtension(new Bt)}))})(),window.order_message=o})();