 ...
 * @license   https://opensource.org/licenses/OSL-3.0 Open Software License (OSL 3.0)
 */
const{$:kt}=window;class jt{extend(t){t.getHeaderContainer().on("click",e.showQuery,(()=>this.onShowSqlQueryClick(t))),t.getHeaderContainer().on("click",e.exportQuery,(()=>this.onExportSqlManagerClick(t)))}onShowSqlQueryClick(t){const n=kt(e.showModalForm(t.getId()));this.fillExportForm(n,t);const o=kt(e.showModalGrid(t.getId()));o.modal("show"),o.on("click",O,(()=>n.submit()))}onExportSqlManagerClick(t){const n=kt(e.showModalForm(t.getId()));this.fillExportForm(n,t),n.submit()}fillExportForm(t,e){const n=e.getContainer().find(r).data("query");t.find('textarea[name="sql"]').val(n),t.find('input[name="name"]').val(this.getNameFromBreadcrumb())}getNameFromBreadcrumb(){const t=kt(m).find(b);let e="";return t.each(((t,n)=>{const o=kt(n),i=o.find("a").length>0?o.find("a").text():o.text();e.length>0&&(e=e.concat(" > ")),e=e.concat(i)})),e}}
/**
 * Copyright since 2007 PrestaShop SA and Contributors
 * PrestaShop is an International Registered Trademark & Property of PrestaShop SA
 *
 ...
 * @license   https://opensource.org/licenses/OSL-3.0 Open Software License (OSL 3.0)
 */
const{$:$t}=window;$t((()=>{const t=new T("attribute_group");t.addExtension(new jt),t.addExtension(new P),t.addExtension(new E),t.addExtension(new M),t.addExtension(new _t),t.addExtension(new Ot),t.addExtension(new Tt),t.addExtension(new Et),t.addExtension(new Lt(t));new At("attributesShowcaseCard").addExtension(new Mt)}))})(),window.attribute_group=o})();