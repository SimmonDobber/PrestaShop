 ...
 * @license   https://opensource.org/licenses/OSL-3.0 Open Software License (OSL 3.0)
 */
const{$:j}=window,B=function(t,e){j.post(t).then((()=>window.location.assign(e)))},{$:M}=window;class T{extend(t){t.getContainer().on("click",p,(t=>{B(M(t.currentTarget).data("url"),M(t.currentTarget).data("redirect"))}))}}
/**
 * Copyright since 2007 PrestaShop SA and Contributors
 * PrestaShop is an International Registered Trademark & Property of PrestaShop SA
 *
 ...
 * @license   https://opensource.org/licenses/OSL-3.0 Open Software License (OSL 3.0)
 */
const{$:St}=window;class $t{extend(t){this.initRowLinks(t),this.initConfirmableActions(t)}initConfirmableActions(t){t.getContainer().on("click",i.linkRowAction,(t=>{const e=St(t.currentTarget).data("confirm-message");e.length&&!window.confirm(e)&&t.preventDefault()}))}initRowLinks(t){St("tr",t.getContainer()).each((function(){const t=St(this);St(i.linkRowActionClickableFirst,t).each((function(){const e=St(this),n=e.closest("td"),o=St(i.clickableTd,t).not(n);let r=!1;o.addClass("cursor-pointer").mousedown((()=>{St(window).mousemove((()=>{r=!0,St(window).unbind("mousemove")}))})),o.mouseup((()=>{const t=r;if(r=!1,St(window).unbind("mousemove"),!t){const t=e.data("confirm-message");(!t.length||window.confirm(t)&&e.attr("href"))&&(document.location.href=e.attr("href"))}}))}))}))}}
/**
 * Copyright since 2007 PrestaShop SA and Contributors
 * PrestaShop is an International Registered Trademark & Property of PrestaShop SA
 *
 ...
 * @license   https://opensource.org/licenses/OSL-3.0 Open Software License (OSL 3.0)
 */
const{$:Pt}=window;Pt((()=>{const e=new x("tax");e.addExtension(new jt),e.addExtension(new A),e.addExtension(new O),e.addExtension(new T),e.addExtension(new $),e.addExtension(new Ct),e.addExtension(new xt),e.addExtension(new Lt),e.addExtension(new At),e.addExtension(new $t),new Ft,new Tt,window.prestashop.component.initComponents(["MultistoreConfigField"]),Pt(t.useEcoTax).on("change",(e=>{const n=Number(Pt(e.currentTarget).val());Pt(t.rowEcoTaxRuleGroup).toggleClass("d-none",0===n)}))}))})(),window.tax=o})();